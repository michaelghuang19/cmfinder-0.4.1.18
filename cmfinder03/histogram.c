...
*
* RCS $Id: histogram.c,v 1.1 2007/01/26 17:24:23 yzizhen Exp $
************************************************************
* Basic API:
* 
	...
	AllocHistogram(int min, int max, int lumpsize)
{
	...
	h->highscore = INT_MIN;
	h->lumpsize  = lumpsize;
	h->histogram = (double *) MallocOrDie (sizeof(double) * newsize);
...
*/
void
AddToHistogram(struct histogram_s *h, float sc, double weight)
{
	int score;
	int moveby;
	int prevsize;
	...
	*/
	if (score < h->min)
	{
		...
		h->histogram = (double *) ReallocOrDie(h->histogram, sizeof(double) * newsize);
		memmove(h->histogram+moveby, h->histogram, sizeof(double) * prevsize);
		for (i = 0; i < moveby; i++)
			h->histogram[i] = 0;
	}
	else if (score > h->max)
	...
	*/
	h->histogram[score - h->min]+= weight;
	h->total+= weight;
...
ExtremeValueFitHistogram(struct histogram_s *h, int censor, float high_hint) 
{
	...
	double *y;                    /* histogram counts            */ 
	double  n;			/* number of observed samples  */
	...
	*/
	for (iteration = 0; iteration < 100; iteration++)
	{
		...
		*/
		...
		y = MallocOrDie(sizeof(double)   * hsize);
		n = 0;
...
{
	double y;
	double p = ExtremeValueP(x,mu,lambda);
	y = N * p;
	if (y < 1e-7) return y;
	if (y > 100 ) return 1;
	return (1.0 - exp(-1. * y));
...
*/ 
void
Lawless416(float *x, double *y, int n, float lambda, float *ret_f, float *ret_df)
{
...
*/ 
void
Lawless422(float *x, double *y, int n, int z, float c,
...
*/
int
EVDMaxLikelyFit(float *x, double *c, int n, float *ret_mu, float *ret_lambda)
{
	float  lambda, mu;
	float  fx;			/* f(x)  */
	float  dfx;			/* f'(x) */
...
*/
int
EVDCensoredFit(float *x, double *y, int n, int z, float c, 
